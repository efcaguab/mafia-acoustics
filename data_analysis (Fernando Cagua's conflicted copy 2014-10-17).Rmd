...
# Old fashioned residency

```{r}

res <- function (enc, remove = NULL, ind.lost = NULL){
  # Residency data frame
residency <- expand.grid (ecocean = unique (enc$ecocean), 
                          date.week = unique (enc$date.week)) %>%
    mutate (ecocean.date = paste (ecocean, date.week), 
            present = ecocean.date %in% enc$ecocean.date)
# Delete observations before the first sighting
residency <- ddply (residency, "ecocean", function (x){
  x <- arrange (x, date.week)
  x$any <- NA
  for (i in 1:nrow (x)){
    x$any[i] <- any (x$present[1:i])
  }
  filter (x, any)
})

# Calculate mean residency
residency <- arrange (residency, date.week)
dates <- unique (residency$date.week)
residency.summary <- data.frame (date.week = dates,
                                 p = NA,
                                 cil = NA, 
                                 ciu = NA)
for (i in 1:length (dates)){
  per.date <- filter (residency, date.week <= dates[i])
  if (!is.null(remove)){
    per.date <- filter (per.date, ecocean != remove)
  }
  if (!is.null (ind.lost)){
    for (j in 1:nrow(ind.lost)){
      per.date <- filter (per.date, !(as.character (ecocean) == as.character(ind.lost$id[j]) & date.week > floor_date (ind.lost$date[j], unit = "week")))
    }
  }
  residency.summary$p[i] <- prop.test (sum (per.date$present), nrow (per.date))$estimate
  residency.summary$cil[i] <- prop.test (sum (per.date$present), nrow (per.date))$conf.int[1]
  residency.summary$ciu[i] <- prop.test (sum (per.date$present), nrow (per.date))$conf.int[2]
}

return (residency.summary)
}

# Acoustically
aco.week <- tbl_df (aco.week)
enc <- filter (aco.week, ecocean %in% filter (ws.tags, batch != "2014-1")$ecocean) %>%
  mutate (ecocean.date = paste (ecocean, date.week))
res.aco <- res (enc, ind.lost = TAG.LOST) %>% mutate (type = "acoustic")
# Visual
enc <- filter (enc.week.tagged, date.week > min (aco.week$date.week)) %>%
  mutate (ecocean.date = paste (ecocean, date.week))
res.vis <- res (enc, remove = "XXX") %>% mutate (type = "visual")

residency <- rbind (res.aco, res.vis)
save (residency, file = "residency_prop.RData")
...
```{r}
# Median tagging date
first.date <- median ((ws.tags %>% filter (batch != "2014-1"))$date.tag) %>% as.Date () -1
last.date <- as.Date ("2014-04-07")
pred.date <- foreach (i=1:length(first.date), .combine = rbind) %do%{
  expand.grid (date.1 = first.date[i], 
                     date.2 = seq (first.date[i], last.date, by ="week"), 
...
pred.date <- cbind (pred.date, predict (men08lme4$gam, pred.date, type = "response", se.fit = TRUE))
