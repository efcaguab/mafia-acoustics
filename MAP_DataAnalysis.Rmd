...
We start by reading the data frames with the ecocean information. We'll merge them all and see what we have. 
Because all input tables have the same structure we can create a function that cleans and process the data in all of them.
...
preprocess.databases <- . %>% 
  # Read file
  read.csv () %>% tbl_df () %>%
  # Select those locations that can be in Mafia/Tanzania/Kilindoni
  filter (grepl ("afia", Locality) | 
            grepl ("nzania", Locality) |
            grepl ("lindon", Locality) |
            Location.ID == "4b") %>%
  # Discard if the indicidual was not identified
  filter (Marked.Individual != "") %>%
  # Calculate date of encounter
  mutate (date = paste (Year.Identified, 
                        Month.Identified, 
                        Day.Identified, 
                        sep = "-"), 
          date = as.Date (date)) %>%
  filter (!is.na (date)) %>% 
  # Use a nicer names
  rename (ecocean = Marked.Individual) %>%
  select (date, ecocean)

uapp.enc <- "./raw-data/unapproved_20150130.csv" %>% preprocess.databases ()
app.enc <- "./raw-data/approved_20150130.csv" %>% preprocess.databases ()
sea.enc <- "./raw-data/approved_20150130.csv" %>% preprocess.databases ()

sightings <- # Join all thre sources of information
  bind_rows (uapp.enc, app.enc, sea.enc) %>%
  # Remove duplicates
  distinct ()

sightings.tagged.sharks <- sightings %>% 
  # Keep only sharks that were tagged during the first season
  inner_join (ws.tags, by = "ecocean") %>%
  filter (batch == "2012-1" | batch == "2012-2") %>%
  select (date, ecocean) %>% 
  arrange (date)
```
...
Residency index is defined as the number of days present/detected divided by the number of days monitored.
The days monitored goes as the last day of download or the date of presumed tag loss, whatever happens first.

```{r, warning=FALSE}
# Acoustic residency index

# Create base data-frame
acoustic.res.index <- det.ws %>%
  # Only include sharks from the first season
  filter (batch == "2012-1" | batch == "2012-2") %>%
  ddply ("ecocean", function (det, tag.lost, end.date){
  # Calculate start date (first detection)
  start.date <- det$datetime %>% min %>% as.Date
  # See when the tag was lost for that shark
  tag.lost.date <- tag.lost %>% filter (ecocean == det$ecocean[1]) %$% date.min[1]
  # Calculate end date (overall monitoring day or tag lost day)
  end.date <- min (end.date %>% as.Date, tag.lost.date, na.rm = T)
  # Generate list of dates for that shark
  data_frame (ecocean = det$ecocean %>% first,
              date = seq (start.date, end.date, by = "day")) %>% 
    return ()
}, tag.lost = tag.lost, end.date = max (det.ws$datetime)) %>% tbl_df () %>%
  # Populate with prescence data from the detections
  left_join (det.ws %>%
  mutate (date = datetime %>% as.Date) %>%
  group_by (ecocean, date) %>%
  summarise (present = n () >= 1)) %>%
  # If the shark was not detected put a 0, 1 otherwise
  mutate (present = replace (present, is.na (present), FALSE), 
          present = present %>% as.numeric) %>% 
  # Calculate a progressive residency index
  ddply ("ecocean", function (x){
    # Arrange by date
    x %<>% arrange (date)
    # For each date
    foreach (i=1:nrow (x), .combine = rbind) %do% {
      x %>% 
        # Filter all dates until then
        slice (1:i) %>% 
        summarise (date = max (date),
                   # Calculate the residency index
                   res.index = sum (present) / n ()) 
    }
  }, .parallel = TRUE) %>% tbl_df ()

acoustic.res.index %>%
  ggplot (aes (x = date, y = res.index)) + 
  geom_line (aes (group = ecocean), alpha = 0.1, colour = "#006F51") +
...
In the graph it's possible to see the indicidual residency indexes for each shark and the smoothed mean value. 
